...
setInterval(updateVisitorCount, 5000); // 5000 milliseconds = 5 seconds
        ...
        document.getElementById("continent-select").addEventListener("change", function () {
        	...
        	// Check if local storage has saved user data
const savedName = localStorage.getItem('name');
const savedEmail = localStorage.getItem('email');

if (savedName && savedEmail) {
document.getElementById('message').innerHTML = `Welcome back, ${savedName}! Your email is ${savedEmail}.`;
}
...
document.getElementById('message').innerHTML = `Hello, ${name}! Your email (${email}) Your number (${num}) has been saved to local storage.`;
});